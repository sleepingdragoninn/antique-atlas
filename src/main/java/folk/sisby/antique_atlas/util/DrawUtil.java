...
public class DrawUtil {
	...
	public static void fill(MatrixStack matrices, VertexConsumerProvider vertexConsumers, RenderLayer layer, float z, int light, int x1, int y1, int x2, int y2, float alpha, float[] color) {
		...
		vertexConsumer.vertex(matrix4f, x1, y1, z).color(color[0], color[1], color[2], alpha).light(light).next();
		vertexConsumer.vertex(matrix4f, x1, y2, z).color(color[0], color[1], color[2], alpha).light(light).next();
		vertexConsumer.vertex(matrix4f, x2, y2, z).color(color[0], color[1], color[2], alpha).light(light).next();
		vertexConsumer.vertex(matrix4f, x2, y1, z).color(color[0], color[1], color[2], alpha).light(light).next();
